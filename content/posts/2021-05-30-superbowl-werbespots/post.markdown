...
Zunächst verschaffe ich mir einen Überblick über die Variablen in dem Datensatz, genauer gesagt schaue ich auf statistische Kennzahlen und Lagemaße. Diese sind für alle Variablen in dem Datensatz in der folgenden Tabelle zusammengefasst.
...
Hier stellt sich die Frage, wie, das heißt, nach welchen Kriterien diese Klassifizierung vorgenommen wurde. Aber für heute nehme ich mal an, dass die Klassifizierung so in Ordnung ist. Anhand der Mittelwerte kann man es schon erkennen: etwa 69% aller Werbespots sind "funny" und 68% zeigen das Produkt kurz ("shop_product_quickly"). Ich habe einmal die TRUE-Werte addiert und durch die Anzahl der Werbespots geteilt: in dieser Mittelwertbetrachtung treffen 2.7 Attribute auf einen Werbespot zu und entsprechend 4.3 Attribute nicht zu. 

Mich interessiert nun: gibt es Gruppen von Werbespots, die entlang dieser sieben Merkmale ähnlich sind? Das ruft nach einer Clusteranalyse (einzelne Beobachtungen werden anhand ausgewählter Variablen zu einem Cluster "gebündelt") und diese habe ich durchgeführt.
...
Nun prüfe ich, wie groß diese 5 Cluster sind, d.h. wieviele Werbespots pro Cluster enthalten sind.
Wünschenswert ist hier eine möglichst gleichmäßige Verteilung. Im Kontrast dazu wäre es z.B. ungünstig, wenn ein Cluster 200 Beobachtungen (Werbespots) enthält, und die restlichen 47 Werbespots verteilen sich auf die restlichen 4 Cluster. In diesem Fall sollte man zumindest einmal stutzig werden und die Clusterung noch einmal anschauen. 
...
Das ist eindeutig: mehr als 100 Werbespots während des Superbowls in den Jahren 2000-2020 stammen von der Brauerei Anheuser Busch (Hersteller der Marken Bud Light und Budweiser). Dann folgen Doritos, Pepsi, Hyundai und Coca-Cola mit 21-25 Werbespots (also pro Jahr durchschnittlich 1 Spot). 
